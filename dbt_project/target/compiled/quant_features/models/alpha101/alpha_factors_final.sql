...
advanced_factors AS (
    SELECT * FROM "quant_features"."main"."alpha_factors_advanced"
),
...
factor_combinations AS (
    ...
    COALESCE(alpha001, 0) - 
    LAG(COALESCE(alpha001, 0), 5) OVER (
        PARTITION BY symbol 
        ORDER BY timestamp
    )

 AS factor_momentum_alpha001,
        
    COALESCE(volume_composite, 0) - 
    LAG(COALESCE(volume_composite, 0), 5) OVER (
        PARTITION BY symbol 
        ORDER BY timestamp
    )

 AS factor_momentum_volume,
        
        -- 因子波动率
        
    STDDEV(COALESCE(alpha003, 0)) OVER (
        PARTITION BY symbol 
        ORDER BY timestamp
        ROWS BETWEEN 19 PRECEDING AND CURRENT ROW
    )
 AS factor_volatility_alpha003,
        
    STDDEV(COALESCE(momentum_composite, 0)) OVER (
        PARTITION BY symbol 
        ORDER BY timestamp
        ROWS BETWEEN 19 PRECEDING AND CURRENT ROW
    )
 AS factor_volatility_momentum,
        
        -- 因子相关性
        
    -- 使用DuckDB的CORR窗口函数
    CORR(COALESCE(alpha001, 0), COALESCE(alpha003, 0)) OVER (
        PARTITION BY symbol 
        ORDER BY timestamp
        ROWS BETWEEN 19 PRECEDING AND CURRENT ROW
    )
 AS factor_corr_mom_rev,
